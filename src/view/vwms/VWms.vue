    ...
    <div class="VWms-header">
      <div class="tooltip">
        <v-btn icon variant="text" @click="handleShowRightContainer">
          <v-tooltip activator="parent" location="bottom">{{ showRightContainer ? 'Close ' : 'Expand ' }}Right column</v-tooltip>
          <v-icon :icon="`mdi-arrow-expand-${showRightContainer?'right':'left'}`"></v-icon>
        </v-btn>

        <v-btn icon variant="text" @click="toggleFullScreen">
          <v-tooltip activator="parent" location="bottom">Full screen</v-tooltip>
        ...
        <v-btn v-if="presentData?.type === 'shelf' || presentData?.type === 'shelfItem'" @click="handleShelfGrid">
          {{ showShelfGrid ? 'Closure ' : 'Open ' }}Shelf 2D view
        </v-btn>
      </div>
    </div>
...
const getWarehouseChatData = () => {
  const barGroupsData = getGroupsData(productInfo.value, 'spu_name', 'qty')
    .slice(0, 6)
  const barTitle = 'Inventory quantity'
  barData.value = {
    barTitle,
    barGroupsData
  }
  ...
  const pieTitle = 'Reservoir area capacity'
  pieData.value = {
    pieTitle,
    xData,
    yData
...
const getFactoryChatData = (usedFactoryShelfItem, factoryShelf) => {
  const barGroupsData = getGroupsData(usedFactoryShelfItem, 'spu_name', 'qty')
    .slice(0, 6)
  const barTitle = 'Amount of inventory in the warehouse area'
  barData.value = {
    barTitle,
    barGroupsData
  }
...
const getShelfData = (id: number): shelfShowDataType => {
  ...
  // 货架二维图数据
  shelfGridData.value = {
    shelf_name: `${ shelf.shelf_name }Shelves`,
    layer: shelf.layer,
    column: shelf.column,
...
const getShelfChatData = (shelfProduct, shelf) => {
  ...
  const barTitle = 'Shelf product specifications'
  barData.value = {
    barTitle,
    barGroupsData
  }
  ...
  const xData = _.map(pieGroupsData, 'name')
  const yData = _.map(pieGroupsData, 'value')
  const pieTitle = 'Location capacity'
  pieData.value = {
    pieTitle,
    xData,
    yData
